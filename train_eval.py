...
# from MODEL.model_2 import SSD300, MultiBoxLoss
# from MODEL.model_3 import SSD300, MultiBoxLoss
# from MODEL.model_5 import SSD300, MultiBoxLoss
# from MODEL.model_2_1 import SSD300, MultiBoxLoss
from MODEL.model_2_3 import SSD300, MultiBoxLoss
...
def main():
    ...
    if checkpoint is None:
        # ###################### model：backbone集成到ssd300里面了#####################
        model = SSD300(n_classes=args.n_classes)
        # ##########################################################################



        # ################### 设置优化器：sgd  多步长衰减学习率 , lr在config中设置 ##############
        ...
        optimizer = torch.optim.Adam(params=[{'params': biases, 'lr': 2 * train_conf.lr},
                                            {'params': not_biases}], lr=train_conf.lr, betas=(0.9, 0.999), eps=1e-08,
                                            weight_decay=0)
        ...
        optim_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                               milestones=[5, 10],
                                                               gamma=0.1)
        # optim_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, 30, eta_min=0.00001, last_epoch=-1)
    # ##########################################################################
    else:
        model = SSD300(n_classes=args.n_classes)
        ...
        # for name, para in model.named_parameters():  # 冻结除以下两个外的所有权重
        #     if "EAO_vis" not in name:
        #         para.requires_grad = False
        #     if "feats_" not in name:
        #         para.requires_grad = False

        # checkpoint = torch.load(checkpoint)
        # start_epoch = checkpoint['epoch'] + 1
        # train_loss = checkpoint['loss']
        # print('\nLoaded che ckpoint from epoch %d. Best loss so far is %.3f.\n' % (start_epoch, train_loss))
        ...
        optimizer = torch.optim.SGD(params=[{'params': biases, 'lr': 2 * train_conf.lr},
                                    ...
                                    weight_decay=train_conf.weight_decay,
                                    nesterov=False)
        optim_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[int(epochs * 0.6)], gamma=0.5)
    # ##########################################################################################################################
    ...
    model = model.to(device)  # 模型放到设备
    model = nn.DataParallel(model)  # 多gpu运行
    criterion = MultiBoxLoss(priors_cxcy=model.module.priors_cxcy).to(device)  # loss放到设备
    # #########################################################################################
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # Save checkpoint
        torch.save(model.module.state_dict(), os.path.join(jobs_dir, 'checkpoint_ssd300{:03d}.pth'.format(epoch)))

        # utils.save_checkpoint(epoch, model, optimizer, train_loss, jobs_dir)
    ...
    for batch_idx, (image_vis, image_lwir, boxes, labels, _) in enumerate(dataloader):
        ...
        if batch_idx % kwargs.get('print_freq', 10) == 0:
            logger.info('Iteration: [{0}/{1}]\t'
                        ...
                        'lr{lr}\t'.format(batch_idx, len(dataloader),
